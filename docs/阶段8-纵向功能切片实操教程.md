...
│  └─ checkpoint.py            # 持久化检查点工厂（支持内存与 SQLite）
├─ api/
│  └─ v1/
│     └─ agent.py              # REST & SSE 接口：触发 Agent 运行、恢复执行与状态查询
└─ core/
...
from app.api.deps import get_facts_factory
...
async def stream_trip_planning(
    request: TripRequest,
    factory: CompositeFactsFactory = Depends(get_facts_factory),
) -> StreamingResponse:
