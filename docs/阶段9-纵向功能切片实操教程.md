    ...
    *,
    ...
    for idx, act in enumerate(sorted_retained):
        ...
        if final_activities:
            ...
            leg = RouteLeg(
                id=leg_id,
                origin_place_id=prev_act.place_id or "origin",
                destination_place_id=act.place_id or "dest",
                mode=request.preferences.transport_modes[0],
                departure_at=prev_act.end_at,
                arrival_at=act_start,
            ...
            act = act.model_copy(update={"route_leg_id": leg_id})
    ...
    stats = DayStatistics(
        ...
        total_duration_minutes=int((current_time - _combine_dt(original_day.date, start_str, tz)).total_seconds() / 60),
        walking_distance_meters=total_walking_meters,
        total_distance_meters=total_walking_meters + 3000,
        total_cost=Money(amount=total_cost_amount, currency=request.display_currency),
...
def calculate_plan_diff(
    ...
    old_walking = sum(day.statistics.walking_distance_meters for day in old_plan.days)
    new_walking = sum(day.statistics.walking_distance_meters for day in new_plan.days)
...
def execute_scoped_replanning(
    *,
    ...
    plan_record = PlanVersionRecord(
        ...
        status=PlanStatus.SUCCEEDED if report.passed else PlanStatus.COMPROMISED,
        trigger=PlanTrigger.REPLANNING,
    ...
    # 8. 保存版本与更新旅行当前指针
    repository.save_plan(plan_record)
...
def test_dynamic_replanning_preserves_unaffected_days_and_generates_diff() -> None:
    ...
    # 4. 验证核心原则：Day 1 与 Day 3 活动 100% 零扰动
    v2_days = body["plan"]["itinerary"]["days"]
    v2_day1_act_ids = [a["id"] for a in v2_days[0]["activities"]]
    v2_day3_act_ids = [a["id"] for a in v2_days[2]["activities"]]

    assert v1_day1_act_ids == v2_day1_act_ids, "Day 1 未受波及，活动 ID 序列必须完全一致！"
    assert v1_day3_act_ids == v2_day3_act_ids, "Day 3 未受波及，活动 ID 序列必须完全一致！"
    ...
    assert len(diff["added_activities"]) > 0 or len(diff["modified_activities"]) > 0
